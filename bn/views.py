...
def ubicar_barcos(request, partida_id):
    """
    ...
    if len(jugador) == 0:
        if not p.iniciada:
            ...
            jugador.tablero = Tablero(ancho=p.ancho_tablero,
    ...
    return render(request, 'ubicacion_barcos.html', {
        'partida': p,
        'error': error,
        'usuario': usuario,
        'form': form,
...
@login_required
@requiere_que_la_partida_exista
def esperando_jugadores(request, partida_id):
    p = get_object_or_404(Partida, pk=partida_id)
    if not p.iniciada:
        faltan = p.cant_jugadores - p.jugador_set.count()
        return render(request, 'esperando_jugadores.html', {
            'partida': p,
...
def esperando_turno(request, partida_id):
    """
    ...
    if jugador.es_su_turno:
        ...
        return render(request, 'esperando_turno.html', {
            'partida': p,
            'tablero': jugador.tablero,
...
@requiere_partida_no_terminada
def elegir_enemigo(request, partida_id):
    """
    ...
    if len(p.jugador_set.exclude(derrotado=True)) > 2:
        ...
        return render(request, 'elegir_enemigo.html', {
            'partida': p,
            'jugadores': jugadores,
            'usuario': request.user,
        })
        ...
        jugador_enemigo = p.jugador_set.exclude(pk=jugador.id).exclude(derrotado=True)[0]
...
@requiere_partida_no_terminada
def ataque(request, partida_id, jugador_id):
    """
    ...
    if request.method == 'POST':  # me fijo si vienen datos
        ...
        if form.is_valid():
            ...
            if error:
                jugadores = p.jugador_set.exclude(derrotado=True)
                return render(request, 'atacar.html', {
                    'form': form,
                    'usuario': request.user,
                    'partida': p,
                    'error': error,
            ...
            return render(request, 'resultados_ataque.html', {
                'form': form,
                'usuario': request.user,
                'partida': p,
                'enemigo': enemigo,
...
@requiere_partida_no_terminada
def defensa(request, partida_id):
    """
            ...
            try:
                if not tipo == 'SD':
                    if tipo == 'E':
                        ...
                        if tipo == 'MC':
                            mijugador.tablero.movimiento_corto(
                                barco, hacia_proa)

                        elif tipo == 'ML':
                            mijugador.tablero.movimiento_largo(
                                barco, hacia_proa)
...
@requiere_partida_iniciada
@requiere_partida_no_terminada
def ganador(request, partida_id):
    """
    ...
    if no_derrotados.count() == 1 and jugador in no_derrotados:
        p.terminada = True
        p.save()
        return render(request, 'ganador.html', {
            'usuario': usuario,
            'partida': p,
            'aciertos': aciertos,
...
@requiere_partida_iniciada
@requiere_partida_no_terminada
def derrotado(request, partida_id):
    """
    ...
    aciertos, celdas_mias_tocadas = jugador.puntos()
    if jugador.derrotado:
        return render(request, 'derrotado.html', {
            'usuario': usuario,
            'partida': p,
            'aciertos': aciertos,
            'celdas_tocadas': celdas_mias_tocadas,
...
@requiere_turno
@requiere_partida_no_terminada
def rendirse(request, partida_id):
    """
...
def resultados(request, partida_id):
    """
...
@requiere_que_la_partida_exista
@requiere_partida_no_terminada
def abandonar(request, partida_id):
    """
