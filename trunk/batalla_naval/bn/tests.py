...
class BNTestCase(TestCase):
    ...
    def crear_tablero(self, jugador):
        ...
        jugador.tablero = Tablero(ancho=partida.ancho_tablero,
...
class CrearPartida(BNTestCase):
    ...
    def test_crea_partida(self):
        ...
        self.assert_(partida.nombre == 'PartidaTest')
        self.assert_(partida.cant_jugadores == 2)
        self.assert_(partida.cant_acorazados == 1)
        self.assert_(partida.cant_patrullas == 1)
        self.assert_(partida.cant_portaaviones == 1)
        self.assert_(partida.cant_submarinos == 1)
        self.assert_(partida.cant_fragatas == 1)
        self.assert_(partida.ancho_tablero == 9)

        self.assert_(partida.cant_turnos_totales == 0)
        self.assert_(partida.cant_turnos_de_la_ronda == 0)
        self.assert_(partida.numero_ronda_actual == 0)
...
class IniciarPartida(BNTestCase):
    def setUp(self):
        ...
        self.jugador_enemigo = self.crear_jugador(
            self.usuario_enemigo, self.partida)
...
class AtaquePotente(BNTestCase):
    ...
    def test_ataque_potente(self):
        ...
        self.assertEqual(self.tablero_enemigo.acorazado_set.all(
        )[0].puntos_tocados, [0, 1])
...
class AtaqueRadar(BNTestCase):
    ...
    def test_ataque_radar(self):
        ...
        self.assertEqual(
            self.tablero_enemigo.acorazado_set.all()[0].puntos_tocados, [])
        self.assertEqual(
            self.tablero_enemigo.fragata_set.all()[0].puntos_tocados, [])
...
class DerrotoOponenteyQuedanOponentes(BNTestCase):
    ...
    def test_derroto_oponente_y_quedan_mas(self):
        ...
        self.assertEqual(
            self.partida.jugador_set.filter(derrotado=False).count(), 2)
...
class MovimientoCorto(BNTestCase):
    ...
    def test_mov_corto(self):
        ...
        response = self.client.get(reverse(
            'bn.views.defensa', args=(self.partida.id,)))
...
class AbandonarPartidaIniciada(BNTestCase):
    ...
    def test_abandonar_partida_iniciada(self):
        ...
        self.assertEqual(
            self.partida.jugador_set.filter(derrotado=True).count(), 1)
        self.assertEqual(
            self.partida.jugador_set.filter(derrotado=False).count(), 1)
